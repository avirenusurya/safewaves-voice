...
class DF_Arena_1B(nn.Module):
    def __init__(self):
        super().__init__()
        config_override = os.getenv("W2V2_XLSR_CONFIG_PATH")
        local_default = Path(__file__).resolve().parent / "wav2vec2_xls_r_1b_config.json"
        config_path = Path(config_override) if config_override else local_default

        if config_path.exists():
            wav2vec_cfg = Wav2Vec2Config.from_json_file(str(config_path))
        else:
            wav2vec_cfg = Wav2Vec2Config.from_pretrained("facebook/wav2vec2-xls-r-1b")

        self.ssl_model = Wav2Vec2Model(wav2vec_cfg)
